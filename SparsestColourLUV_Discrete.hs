...
take2 :: [a] -> [(a,a)]
take2 []     = []
take2 (x:xs) = map ((,) x) xs ++ take2 xs where

distGraph :: [RGBColour] -> Double
distGraph cs = minimum (fmap f (take2 cs)) where
...
stepGraph cs = do
	...
	pure $ maximumBy (comparing (liftA2 (,) distGraph varGraph)) $ do
		o' <- adjacentColours o
		pure $ o' : os
...
randomColour :: IO RGBColour
randomColour = let
	random8bit = randomRIO (0x00, 0xff)
	in liftA3 Vector3 random8bit random8bit random8bit

initialGraph :: IO ([RGBColour], Double, Double)
initialGraph = do
	str <- readFile outputPath
	let (cs, [distSq,var]) = splitAt coloursN (lines str)
	pure ((\(Colour.RGB r g b) -> Vector3 r g b) . Colour.toSRGB24 . Colour.sRGB24read <$> cs, read distSq, read var)

maxIter :: Int
maxIter = 256 * coloursN

iterGraph :: Int -> Double -> Double -> [RGBColour] -> IO [RGBColour]
iterGraph iter record recordV cs = if (record, recordV) < (distGraph cs, varGraph cs)
	then return cs
	else if iter < maxIter
		then stepGraph cs >>= iterGraph (iter+1) record recordV
		else return []

findGraph :: MVar [RGBColour] -> IO ()
findGraph result = do
	(initGraph, distSq, var) <- initialGraph
	resultGraph <- iterGraph 0 distSq var initGraph
	case resultGraph of
		[] -> findGraph result
...
main = do
	...
	sequence_ $ replicate (threadN - 1) (forkIO (findGraph result))
	cs <- readMVar result
	writeFile outputPath ""
	for_ cs (\c -> putStrLn (showColour c) >> appendFile outputPath (showColour c ++ "\n"))
	let distSq = distGraph cs
	let var = varGraph cs
	putStrLn $ show distSq
	putStrLn $ show var
	appendFile outputPath (show distSq ++ "\n")
	appendFile outputPath $ show var
  where
